...
</main>`;let f;const S=new Uint8Array(16);function L(){if(!f&&(f=typeof crypto<"u"&&crypto.getRandomValues&&crypto.getRandomValues.bind(crypto),!f))throw new Error("crypto.getRandomValues() not supported. See https://github.com/uuidjs/uuid#getrandomvalues-not-supported");return f(S)}const i=[];for(let t=0;t<256;++t)i.push((t+256).toString(16).slice(1));function A(t,e=0){return(i[t[e+0]]+i[t[e+1]]+i[t[e+2]]+i[t[e+3]]+"-"+i[t[e+4]]+i[t[e+5]]+"-"+i[t[e+6]]+i[t[e+7]]+"-"+i[t[e+8]]+i[t[e+9]]+"-"+i[t[e+10]]+i[t[e+11]]+i[t[e+12]]+i[t[e+13]]+i[t[e+14]]+i[t[e+15]]).toLowerCase()}const C=typeof crypto<"u"&&crypto.randomUUID&&crypto.randomUUID.bind(crypto),v={randomUUID:C};function j(t,e,r){if(v.randomUUID&&!e&&!t)return v.randomUUID();t=t||{};const n=t.random||(t.rng||L)();if(n[6]=n[6]&15|64,n[8]=n[8]&63|128,e){r=r||0;for(let s=0;s<16;++s)e[r+s]=n[s];return e}return A(n)}class y{constructor(e,r){this.id=j(),this.title=e,this.status=!!r,this.createdAt=new Date}}const D=(t,e)=>{const r=document.querySelector(t);r.innerHTML="",e.forEach(n=>{r.innerHTML+=b(n)})},I=(t,e)=>{const r=document.querySelector(t);r.innerHTML+=b(e)},b=t=>{if(!t instanceof y)throw new Error("Invalid Todo instance");const e=t.status?"fa-solid fa-check mr-2 cursor-pointer text-green-500":"fa-solid fa-clock mr-2 text-yellow-500";return`<div data_id=${t.id} class="bg-stone-900 hover:bg-stone-700 hover:transition-all transition-all cursor-pointer rounded-md my-2 flex items-center justify-between">
...
</div>`},q=(t,e,r,n)=>{if(!Array.isArray(n))throw new Error("Invalid todos");document.querySelector(t).innerText=n.filter(s=>s.status).length,document.querySelector(e).innerText=n.filter(s=>!s.status).length,document.querySelector(r).innerText=n.length},E=t=>{window.innerWidth<=1020?iziToast.info({title:"Add Task",message:`${t}`,position:"topCenter"}):iziToast.info({title:"Add Task",message:`${t}`,position:"bottomRight"})},_=t=>{window.innerWidth<=1020?iziToast.success({title:"Change status",message:`${t}`,position:"topCenter"}):iziToast.success({title:"Change status",message:`${t}`,position:"bottomRight"})},w=t=>{window.innerWidth<=1020?iziToast.success({title:"Delete",message:`${t}`,position:"topCenter",backgroundColor:"#F06666"}):iziToast.success({title:"Delete",message:`${t}`,position:"bottomRight",backgroundColor:"#F06666"})},m={All:"all",Completed:"completed",Pending:"pending"},a={todos:[],filter:m.All},U=()=>{T(),console.log("initStore 🧻")},T=()=>{if(!localStorage.getItem("state"))return;console.log(JSON.parse(localStorage.getItem("state")));const{todos:t=[],filter:e=m.All}=JSON.parse(localStorage.getItem("state"));a.todos=t,a.filter=e},p=()=>{localStorage.setItem("state",JSON.stringify(a))},$=(t=m.All)=>{switch(t){case m.All:return[...a.todos];case m.Completed:return[...a.todos].filter(e=>e.status);case m.Pending:return[...a.todos].filter(e=>!e.status);default:throw new Error("Invalid filter: "+t)}},F=t=>{if(!t)throw new Error("Title is required");a.todos.push(new y(t,!1)),p(),_("Add Task Successfully")},O=t=>{a.todos=a.todos.map(e=>(e.id===t&&(_(e.status?"Task pending":"Task completed"),e.status=!e.status),e)),p()},N=t=>{a.todos=a.todos.filter(e=>e.id!==t),w("Delete Task Successfully"),p()},P=()=>{if(a.todos.filter(t=>t.status).length===0){E("You dont have completed tasks");return}a.todos=a.todos.filter(t=>!t.status),w("Delete Completed Tasks Successfully"),p()},z=(t=m.All)=>{a.filter=t,p()},R=()=>a.filter,c={initStore:U,loadStore:T,getTodos:$,addTodo:F,toggleTodo:O,deleteTodo:N,getCurrentFilter:R,setFilter:z,deleteCompleted:P,Filters:m},d={todoList:"#todo-list",text_input:"#task_input",btn_add:"#btn_add",error_text:"#error_text",count_done:"#count-done",count_pending:"#count-pending",count_all:"#count-all",clear_completed:"#clear-completed",task_states:"#task-states"},H=t=>{const e=()=>{const l=c.getTodos(c.getCurrentFilter());D(d.todoList,l);const u=c.getTodos(c.Filters.All);q(d.count_done,d.count_pending,d.count_all,u)},r=()=>{const l=c.getTodos(c.getCurrentFilter());let u=l[l.length-1];I(d.todoList,u)};(()=>{const l=document.createElement("div");l.innerHTML=k,document.querySelector(t).append(l),e()})();const n=document.querySelector(d.text_input),s=document.querySelector(d.btn_add),o=document.querySelector(d.error_text),g=30,x=3;n.addEventListener("keyup",l=>{n.classList.add("text-green-600"),n.value.trim().length>g||n.value.trim().length<x?n.classList.replace("text-green-600","text-red-500"):n.classList.replace("text-red-500","text-green-600")}),s.addEventListener("click",l=>{if(l.preventDefault(),n.value.trim().length>g)o.classList.remove("hidden"),o.innerText=`Task must be less than ${g} characters`;else if(n.value.trim().length<x)o.classList.remove("hidden"),o.innerText=`Task must be more than ${x} characters`;else{o.classList.add("hidden");const u=n.value.trim();c.addTodo(u),r()}}),document.querySelector(d.todoList).addEventListener("click",l=>{const u=l.target.closest("[data_id]");if(u===null)return;let h=u.getAttribute("data_id");if(l.target.classList.contains("fa-trash")){c.deleteTodo(h),console.log("delete"),e();return}else c.toggleTodo(h),e()}),document.querySelector(d.clear_completed).addEventListener("click",l=>{c.deleteCompleted(),e()}),document.querySelector(d.task_states).addEventListener("click",l=>{let u=String(l.target.closest("[data-state]").getAttribute("data-state"));c.setFilter(u),e()})};c.initStore();H("#app");
